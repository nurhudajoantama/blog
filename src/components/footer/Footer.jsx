...
function FooterList({ title, linkList, thisWeb }) {
  function LinkOtherWeb({ title, link }) {
    return (
  ...
  function LinkThisWeb({ title, link }) {
    return (
      <AniLink paintDrip title={title} to={link}>
        {title}
      </AniLink>
    );
  }
  ...
  return (
    <div className="md:mx-10 mb-7">
      <h3 className="font-semibold mb-3">{title}</h3>
      <ul>
        {linkList.map((footerLink, index) => (
          <li key={index}>{thisWeb ? <LinkThisWeb title={footerLink.name} link={footerLink.link} /> : <LinkOtherWeb title={footerLink.name} link={footerLink.link} />}</li>
        ))}
      </ul>
    </div>
  );
...
export default function Footer() {
  ...
  const gunadarmaLinks = [
    {
      name: "Gunadarma Homepage",
      link: "https://gunadarma.ac.id",
    },
    {
      name: "Perpustakaan Gunadarma",
      link: "https://library.gunadarma.ac.id/",
    },
    {
    ...
    },
    {
      ...
      link: "http://ti.lab.gunadarma.ac.id/",
    },
  ];
  return (
    <div className="bg-blue-700 min-h-1/3 py-3 mt-7">
      <div className="my-10 p-3 container min-h-48 flex flex-col md:flex-row justify-between text-white">
        <p className="text-white flex-grow self-center order-last md:order-first">&copy; 2021 Nurhuda Joantama Putra. All Right Reserved</p>
        <FooterList title="Link" linkList={footerLinks} thisWeb />
        <FooterList title="Gunadarma Link" linkList={gunadarmaLinks} />
        <SocialMediaFooter />
      </div>
    </div>
  );
